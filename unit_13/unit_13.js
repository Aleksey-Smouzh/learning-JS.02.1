...
function f6(arr, block) {
  ...
  document.querySelector(".b-6").onclick = f5(a6, ".out-6");
}
...
function f12() {
  let k = document.querySelector(".i-12").value;
  console.log(a12);
  for (let key in a12) {
    if (a12[key] == k) {
      delete a12[key];
...
function f19() {
  let out = "";
  let inp = document.querySelector(".i-19").value.toLowerCase();
  for (let key in a19) {
    for (let i = 0; i < a19[key].length; i++) {
      if (a19[key][i].toLowerCase() == inp) {
        out += key;
      }
    }
  }
